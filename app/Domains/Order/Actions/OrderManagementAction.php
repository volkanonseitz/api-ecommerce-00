...
class OrderManagementAction
{
    ...
    public function execute(OrderData $data, Settings $settings, ?User $user = null): Order
    {
        return DB::transaction(function () use ($data, $settings, $user) {
            ...
            // 6. Menyimpan Parent Order melalui CreateOrderAction Anda
            $order = $this->createOrderDb->execute($data);
            ...
            if (! in_array($order->payment_gateway, [
                PaymentGatewayType::CASH->value, PaymentGatewayType::CASH_ON_DELIVERY->value, PaymentGatewayType::FULL_WALLET_PAYMENT->value,
            ], true)) {
