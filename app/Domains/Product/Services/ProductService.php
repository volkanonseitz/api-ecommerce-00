...
class ProductService
{
    ...
    public function getProductDetail(Request $request, string $slug): Product
    {
        ...
        if ($request->has('with') && (str_contains((string) $request->with, 'digital_file') || str_contains((string) $request->with, 'variation_options.digital_file'))) {
            if (! $this->hasPermission($user, $product->shop_id)) {
                throw new \Exception(config('notice.NOT_AUTHORIZED'));
            }
        }

        $related = $this->getRelatedProducts($product, (int) $limit, $language);
     ...
     * @return LengthAwarePaginator<Product>
     */
    public function getDraftedProducts(Request $request): LengthAwarePaginator|Builder
    {
        $user = $request->user();
        $language = $request->language ?? config('shop.default_language', 'id');
     ...
     * @return LengthAwarePaginator<Product>
     */
    public function getProductStock(Request $request): LengthAwarePaginator|Builder
    {
        $user = $request->user();
        $language = $request->language ?? config('shop.default_language', 'id');
        $query = Product::with(['type', 'shop'])
